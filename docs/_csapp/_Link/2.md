...
当编译器遇到一个不在当前模块的符号 (变量 / 函数名)，为之生成标记，然后汇编器生成符号表条目，把它交给 linker 进行符号解析: 把每个引用和所有输入的 `.symtab` 中的一个符号定义关联起来。

#### (1). 符号类型

每个目标模块都有一个符号表，在链接器的上下文中，有三种符号:
...
-   strong: &ensp; 函数和已初始化的全局变量
-   weak: &ensp; 未初始化的全局变量 (`gcc 10.2` 中这个也是强符号)

Linux 链接器根据如下规则处理多重定义的符号:
...
-   重定位节中的符号引用
    -   linker 修改代码节和数据节中对每个符号的引用，使它们只想正确的运行时地址，这一步依赖于 relocation entry
