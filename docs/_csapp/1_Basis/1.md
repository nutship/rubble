...
> 以 8-bit 数为例， <br> 
补码: $((-2) + (-4)) \bmod 256 = (254 + 252) \bmod 256 = 250 \bmod 256,\ f_{tc}(250)=-6$ <br>
反码: $((-2) + (-4)) \bmod 255 = (253 + 251) \bmod 255 = 249 \bmod 255,\ f_{oc}(249)=-6$

基于此，用区间内的大数表示负数，负数通过加上模数可以转化为原数，因此区间映射必须是如下形式<br>
$\qquad \qquad [0, 255] \  \longrightarrow\ [0, ..., x, -y, -y+1, ...]$ <br>
$X$ 的取值影响正负数的范围。如果令 $x=127,\ y=x+1$，模数 $=256$，就使得:

- 正负数范围均衡，且所有 $a$ 和 $-a$ 底层表示之间恰好满足取反加一的关系 (和为 256)
- 相比反码 (one's complement, $x=y=127$，模数 $=255$)，$0$ 的表示唯一
