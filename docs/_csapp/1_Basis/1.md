...
补码 (two's complement) 运用了同余的思想，可以正确计算加减乘法。<br>
$\qquad a\equiv b \pmod m,\ c \equiv d \pmod m \ \Longrightarrow \ a \pm /\* c \equiv b \pm / \* d \pmod m$ <br>
以 8 位为例，任何数都可根据同余映射到 $[0, 255]$，因此只需构建一个映射<br>
$\qquad \qquad [0, 255] \  \longrightarrow\ [0, ..., x, -(x+1), ... , -1]$ <br>
即可实现有符号数的表示 和 加减乘运算。一般 $x=01...1$，补码可简记为取反加一 或 最高位负权求和

> 例: $(-2) + (-4) \xrightarrow{represent} (254 + 252) \bmod 256 \xrightarrow{result} 250 (-6) $

反码 (one's complement) 的思想是基于 127 同余，设符号位表示，由于 0 的表示不唯一而被舍弃。<br>
由于衡量的是和模之间的距离，因此不论是扩展有符号数的位表示，还是算数右移 `>>`，都需要复制最高位。
...
<img src="../img/1bitrange.png" width="400">
